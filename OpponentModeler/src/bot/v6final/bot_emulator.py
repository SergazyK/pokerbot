...
class BotEmulatorPlayer(BasePokerPlayer):
    def __init__(self, model_num=None, bot_name = None):
        model_dir = 'models/' + bot_name
        super().__init__()
        names = [bot_name, bot_name]

        self.model_call = MergeModel(len(names), model_dir, 'call')
        self.model_raise = MergeModel(len(names), model_dir, 'raise')
        self.model_raise_amount = MergeModel(len(names), model_dir, 'raise_amount', classifier=CatBoostRegressor)
